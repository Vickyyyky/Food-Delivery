...
exports.updateFoodImage = AsyncHandler(async (req, res) => {
  ...
  if (public_id !== undefined || public_id !== null) {
    let deletedimage = await deleteImgFromCloudinary(public_id);
  }
  let localFilePath = req?.file?.path;
  let uploadedResponse = await uploadImgOnCloudinary(localFilePath);
...
exports.getSingleFood = AsyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json({ success: true, message: "food fetched succesfully", food });
  // res.send(food);
});
